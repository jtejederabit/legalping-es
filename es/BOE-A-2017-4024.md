...
a) Para NOx: la concentración de NOx se expresará como NO2. Las concentraciones respectivas de NO y NO2 expresadas en ppm se sumarán y al valor resultante se le aplicará el coeficiente 2,054.
...
Siendo S2 el azufre emitido, en peso, medido en chimenea durante el mes y S1 el azufre, en peso, que tenga el combustible de entrada en la instalación GIC consumido en ese mes antes de someterlo a algún proceso específico para su desulfuración.
...
6. Emisión trimestral (de SO2, NOx y partículas) (tT). Masa total de un contaminante emitida trimestralmente a la atmósfera. Se expresa en toneladas y se obtiene multiplicando la concentración media trimestral de dicho contaminante por el volumen trimestral de gases generado. Es decir:

donde:

CT: Concentración media trimestral, en mg/Nm3, según se ha definido en el epígrafe 3 del apartado A de este anexo III.

VT: Volumen total trimestral de gases de emisión en condiciones normales de presión y temperatura (en Nm3 x 103), referido a las mismas condiciones de humedad y exceso de oxígeno que CT.
...
En el caso del gas natural, se podría utilizar la siguiente expresión para el cálculo del VES basada en el PCI

VES en condiciones normales de presión y temperatura (Nm3/kg combustible) = 0,64972 + 0,22553 x PCI (kJ/kg).

El volumen total (VT o VS) necesario para calcular la emisión total (tT o tS) se obtendrá del modo siguiente:

siendo % X el oxígeno de referencia correspondiente.

El volumen de gases VG así obtenido estará referido a base seca y al oxígeno de referencia aplicable. Multiplicando VG por la masa de combustible consumido durante el periodo de cómputo (trimestre o semestre) se obtiene el Volumen total correspondiente.
